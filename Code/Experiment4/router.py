...
class RouterNode:
    ...
    def _task_hello(self):
        ...
        while self.running:
            ...
            # 向所有激活端口广播
            for port in list(self.active_ports.keys()): # Copy keys
    ...
    def _initiate_send(self, target_id, msg):
        ...
        with self.rt_lock:
            ...
            print(f"[发送] 目标:{target_id} 下一跳:{route['next_hop_id']} ({port})")
            self._send_to_port(port, packet)
